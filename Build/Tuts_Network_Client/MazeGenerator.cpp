...
void MazeGenerator::interpret(string m) {
	//cout << m << endl;
	string integers = "";
	int parser = 1;
	
	for (uint i = 6; m.at(i) != '_'; i++) {
		integers += m.at(i);
		parser = i + 1;
	}
	size = stoi(integers);
	...
	++parser;
	int secondParser;
	for (uint i = 0; i < size * (size - 1) * 2 ; ++i) {
		if (m.at(parser) == '1') allEdges[i]._iswall = true;
		else allEdges[i]._iswall = false;
		parser++;
	}
	...
	/*for (uint y = 0; y < size; ++y)
	{
		for (uint x = 0; x < size - 1; ++x)
		{
			...
			edgeX._b = &allNodes[y * size + x + 1];
			edgeX.weighting = 1.0f;
			char cmpr = m.at(parser + 1 + (y * (size - 1) + x));
			secondParser = parser + 1 + (y * (size - 1) + x) + 1;
			if (cmpr == '1') {
				edgeX._iswall = true;
			}
			else {
	...
	for (uint y = 0; y < size - 1; ++y)
	{
		for (uint x = 0; x < size; ++x)
		{
			...
			edgeY._b = &allNodes[(y + 1) * size + x];
			edgeY.weighting = 1.0f;
			char cmpr = m.at(secondParser + 1 + (y * (size - 1) + x));
			parser = (secondParser + 1 + (y * (size - 1) + x));
			if (cmpr == '1') {
				edgeY._iswall = true;
			}
			else {
	...
	/*for (int i = parser; m.at(i) != '|'; i++) {
		secondParser = i + 1;
	}
	for (int i = secondParser; m.at(i) == '|'; i++) {
		parser = i + 1;
	}
	...
	start->_pos.x = stoi(integers);
	integers = "";
	for (int i = secondParser; m.at(i) != '|'; i++) {
		integers += m.at(i);
		parser = i + 2;
	}
	start->_pos.y = stoi(integers);
	integers = "";
	//int parser = 1;
	for (int i = parser; m.at(i) != ','; i++) {
		integers += m.at(i);
	...
	end->_pos.x = stoi(integers);
	integers = "";
	for (int i = secondParser; m.at(i) != '|'; i++) {
		integers += m.at(i);
		parser = i + 2;
	}
	end->_pos.y = stoi(integers);*/
}

string MazeGenerator::CreateCoordPacket() {
	string s = "";
	s += GetCoordinates(start);
	s += "|";
	s += GetCoordinates(end);
	s += "|";
	return s;
}

string MazeGenerator::GetCoordinates(GraphNode* node) {
	string s = "";
